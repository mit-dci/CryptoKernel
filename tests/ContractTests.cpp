...
bool ContractTest::runScript(const std::string contract) {
    ...
    std::unique_ptr<CryptoKernel::Storage::Transaction> blockchainHandle(
        blockchain->getTxHandle());
...
void ContractTest::testCrypto() {
    const std::string contract =
        "local genCrypto = Crypto.new(true) local publicKey = genCrypto:getPublicKey() local signature = genCrypto:sign(\"this is a test signature\") local crypto = Crypto.new() crypto:setPublicKey(publicKey) if crypto:verify(\"this is a test signature\", signature) then return true else return false end";
...
void ContractTest::testAccessTx() {
    const std::string contract =
        "if thisTransaction[\"inputs\"][1][\"nonce\"] == 272727 then return true else return false end";
...
void ContractTest::testVerifySignature() {
    const std::string contract =
        "local crypto = Crypto.new() crypto:setPublicKey(thisTransaction[\"inputs\"][1][\"publicKey\"]) if crypto:verify(thisTransaction[\"inputs\"][1][\"id\"] .. outputSetId, thisTransaction[\"inputs\"][1][\"signature\"]) then return true else return false end";
...
void ContractTest::testBlockchainAccess() {
    const std::string contract =
        "local json = Json.new() local tip = json:decode(Blockchain.getBlock(\"tip\")) local currentId = tip[\"id\"] for currentHeight = tip[\"height\"], 2, -1 do local nextBlock = json:decode(Blockchain.getBlock(currentId)) currentId = nextBlock[\"previousBlockId\"] end if currentId == \"584ce0855260bb4cbb3b093fe892c747f4b699dcaff247c35ba55dd255d211a1\" then return true else return false end";
