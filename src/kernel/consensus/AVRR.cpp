...
bool CryptoKernel::Consensus::AVRR::isBlockBetter(const CryptoKernel::Blockchain::dbBlock& block, const CryptoKernel::Blockchain::dbBlock& tip) {
    return (block.getConsensusData()["sequenceNumber"].asUInt64() < tip.getConsensusData()["sequenceNumber"].asUInt64() && block.getHeight() >= tip.getHeight());
}

std::string CryptoKernel::Consensus::AVRR::serializeConsensusData(const CryptoKernel::Blockchain::block& block) {
    return block.getConsensusData()["publicKey"].asString() + block.getConsensusData()["sequenceNumber"].asString();
}
...
bool CryptoKernel::Consensus::AVRR::checkConsensusRules(const CryptoKernel::Blockchain::block& block, const CryptoKernel::Blockchain::block& previousBlock) {
    ...
    if(blockData.sequenceNumber <= previousBlockData.sequenceNumber || (block.getTimestamp() / blockTarget) != blockData.sequenceNumber) {
        return false;
    }
