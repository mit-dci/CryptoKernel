...
LUA_LIBDIR ?= /usr/lib
LIBFLAGS   ?= -shared
BINFLAGS   ?= -I. -L. -L/usr/local/lib -Wl,-rpath -Wl,./ -lck -llua5.3 -lcrypto -ldl -pthread -lleveldb -lmicrohttpd -ljsonrpccpp-common -ljsonrpccpp-server -ljsonrpccpp-client -lcurl -lgcov -ljsoncpp -lsfml-network -lsfml-system
STATICBINFLAGS   ?= -I. -L. -L/usr/local/lib -llua5.3 -lcrypto -ldl -pthread -lleveldb -lmicrohttpd -ljsonrpccpp-common -ljsonrpccpp-server -ljsonrpccpp-client -lcurl -lgcov -ljsoncpp -lsfml-network -lsfml-system 
...
KERNELSRC = src/kernel/blockchain.cpp src/kernel/blockchaintypes.cpp src/kernel/math.cpp src/kernel/storage.cpp src/kernel/network.cpp src/kernel/networkpeer.cpp src/kernel/base64.cpp src/kernel/crypto.cpp src/kernel/log.cpp src/kernel/contract.cpp src/kernel/consensus/AVRR.cpp src/kernel/consensus/PoW.cpp src/kernel/merkletree.cpp src/kernel/consensus/regtest.cpp
...
static: $(CLIENTSRC) $(KERNELSRC) $(LYRASRC) $(CLIENTOBJS) $(KERNELOBJS) $(LYRAOBJS)
	$(CC) $(CLIENTOBJS) $(KERNELOBJS) $(LYRAOBJS) -o ckd -L$(LUA_LIBDIR) $(STATICBINFLAGS)
